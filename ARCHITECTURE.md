...
- Box log streaming: API exposes box-scoped SSE logs (`/v1/boxes/:boxId/logs`), forwards `follow/since/tail` to orchestrator runtime log streams, and aborts downstream Docker follow streams when clients disconnect.
...
- Box cleanup: one shared idempotent cleanup path handles create-failure compensation, remove flows, and external container deletion cleanup jobs (Tailnet device + container + network + volume).
- External container deletion: reconciliation enqueues a cleanup job instead of hard-deleting immediately.
- Runtime entrypoint (`docker/runtime/dev-entrypoint.sh`): uses fixed state dir (`/workspace/.tailscale`), fails fast when neither authkey nor persisted state exists, reconnects with Tailscale-only SSH, and applies iptables firewall (allow lo + established + tailscale0, drop rest).
