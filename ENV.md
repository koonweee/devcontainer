...
- Deployment: set required-by-context values when defaults do not match your network or storage layout.

## Required by context
- `DEVBOX_RUNTIME_IMAGE`
  - Set this in deployment to a published workspace image tag or digest.
- `DEVBOX_PUBLIC_API_URL`
  - Set this when browser clients cannot reach API at the default.
- `DEVBOX_INTERNAL_API_URL`
  - Set this when web server-side calls should target service DNS or an internal network.
- `DEVBOX_API_URL`
...
- `DEVBOX_RUNTIME_IMAGE` (default: `devbox-runtime:local`)
  ...
  - Recommendation: build and tag from `docker/runtime/Dockerfile` before starting the API (`npm run build:runtime-image`).
...
- `DEVBOX_WEB_ORIGIN` (default: `http://localhost:5173,http://localhost:4173`)
  ...
  - Recommendation: include every browser origin that should call the API directly.

## Runtime container env (injected by orchestrator)
These are orchestrator-managed env vars injected into each created workspace container.

- `DEVBOX_TAILSCALE_AUTHKEY` - Per-box Tailscale auth key minted at create time and not persisted in orchestrator state.
- `DEVBOX_TAILSCALE_HOSTNAME` - Tailscale hostname for the box (for example `devbox-mybox-a1b2c3d4`).
- `DEVBOX_TAILSCALE_STATE_DIR` - Optional Tailscale state directory override used by the runtime entrypoint (default: `/var/lib/tailscale`).
